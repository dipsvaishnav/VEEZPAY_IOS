...
public typealias MultiSelectionClosure = ([DropDownIndex], [String]) -> Void
...
public final class DropDown: UIView {
  ...
  fileprivate static var bundle: Bundle {
	...
	*/
	...
	fileprivate var selectedRowIndices = Set<DropDownIndex>()
...
extension DropDown {
	...
	*/
	...
	public func selectRow(at index: DropDownIndex?, scrollPosition: UITableView.ScrollPosition = .none) {
		if let index = index {
            tableView.selectRow(
    ...
    public func selectRows(at indices: Set<DropDownIndex>?) {
        indices?.forEach {
            selectRow(at: $0)
        }
	...
	public func deselectRow(at index: DropDownIndex?) {
    ...
    public func deselectRows(at indices: Set<DropDownIndex>?) {
        indices?.forEach {
            deselectRow(at: $0)
        }
    }
	...
	@objc public func selectRow(_ index: Int, scrollPosition: UITableView.ScrollPosition = .none) {
        self.selectRow(at:DropDownIndex(index), scrollPosition: scrollPosition)
    }
    ...
    @objc public func deselectRow(_ index: Int) {
        tableView.deselectRow(at: IndexPath(row: DropDownIndex(index), section: 0), animated: true)
    }
